...
function createRateLimiter(
  ...
  if (
    ...
    !isFinite(refillRatePerSecond) ||
    refillRatePerSecond <= 0
  ) {
    throw new TypeError('refillRatePerSecond must be a finite positive number');
  }
  ...
  function consume(tokens: number = 1): boolean {
    if (
      ...
      throw new TypeError('tokens must be a non-negative finite number');
    }
