export function mapWithConcurrency<T, R>(
  ...
  const typedFn = fn as (item: T, index: number, array: T[]) => Promise<R>;
  ...
  async function worker(): Promise<void> {
    while (index < typedItems.length && !rejected) {
      const currentIndex = index++;
      const item = typedItems[currentIndex];
      try {
        const result = await typedFn(item, currentIndex, typedItems);
