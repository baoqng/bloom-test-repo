...
function chunk<T>(arr: unknown, size: unknown): T[][] {
  ...
  if (typeof size !== 'number' || !isFinite(size)) {
    throw new TypeError(`Expected size to be a number, got ${size === null ? 'null' : typeof size}`);
  }

  if (!Number.isInteger(size)) {
    throw new TypeError(`Expected size to be an integer, got ${size}`);
  }

  if (size < 1) {
    throw new RangeError(`Expected size to be a positive integer (>= 1), got ${size}`);
  }
