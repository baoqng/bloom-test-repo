...
export function calculateInvoiceTotal(invoice: {
  ...
  if (globalDiscount !== undefined) {
    ...
    const discountObj = globalDiscount as Record<string, unknown>;
    const dtype = discountObj['type'];
    const dvalue = discountObj['value'];
    ...
    if (
      typeof dvalue !== 'number' ||
      !Number.isFinite(dvalue) ||
      dvalue < 0
    ) {
    ...
    if (dtype === 'percentage') {
      const computed = round2((dvalue / 100) * invoiceSubtotal);
      discountAmount = Math.min(computed, invoiceSubtotal);
      discountAmount = round2(discountAmount);
    } else {
      discountAmount = round2(Math.min(dvalue, invoiceSubtotal));
    }
  }
  ...
  const total = round2(discountedSubtotal + totalTaxAmount);
