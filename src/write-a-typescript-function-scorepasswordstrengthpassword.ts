...
function scorePasswordStrength(password: unknown): { score: 0 | 1 | 2 | 3 | 4, label: 'very-weak' | 'weak' | 'fair' | 'strong' | 'very-strong', feedback: string[] } {
  ...
  // If score zero rule applies, force score to 0
  if (isScoreZero) {
    score = 0;
  } else {
    // Clamp to [0, 4]
  ...
  // Feedback — evaluated independently of Score 0 rules
  let feedback: string[] = [];
