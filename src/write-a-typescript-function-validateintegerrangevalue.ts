...
function validateIntegerRange(value: unknown, min: unknown, max: unknown): number {
  ...
  // Validate min
  if (typeof min !== 'number' || Number.isNaN(min) || !Number.isFinite(min) || !Number.isInteger(min)) {
    throw new TypeError('min must be an integer');
  }

  // Validate max
  if (typeof max !== 'number' || Number.isNaN(max) || !Number.isFinite(max) || !Number.isInteger(max)) {
