...
export function applyPriorityScore(
  urgency: unknown,
  impact: unknown,
  effort: unknown
): { score: number; priority: 'critical' | 'high' | 'medium' | 'low' } {
  if (typeof urgency !== 'number' || Number.isNaN(urgency)) {
    throw new TypeError('urgency must be a number');
  }
  if (!Number.isFinite(urgency) || urgency < 1 || urgency > 10) {
  ...
  if (typeof effort !== 'number' || Number.isNaN(effort)) {
