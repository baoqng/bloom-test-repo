...
function applyCircuitTransition(
  ...
  const state = trimmedState as 'CLOSED' | 'OPEN' | 'HALF_OPEN';

  if (state === 'CLOSED' && failureCount >= failureThreshold) {
    return 'OPEN';
  }

  if (state === 'OPEN' && successCount > 0) {
    return 'HALF_OPEN';
  }
  ...
  if (state === 'HALF_OPEN' && failureCount > 0) {
    return 'OPEN';
  }
