...
function generateSlug(input: unknown, options?: { maxLength?: number; separator?: string; lowercase?: boolean }): string {
  ...
  const notAlphanumOrSep = new RegExp(`[^a-zA-Z0-9${escapedSep}]`, 'g');
  result = result.replace(notAlphanumOrSep, separator);

  // Step 4: collapse consecutive separators into one
  const consecutiveSep = new RegExp(`${escapedSep}+`, 'g');
  result = result.replace(consecutiveSep, separator);

  // Step 5: strip leading and trailing separators
  const leadingTrailingSep = new RegExp(`^${escapedSep}+|${escapedSep}+$`, 'g');
  result = result.replace(leadingTrailingSep, '');
