  ...
  switch (type) {
    ...
    case 'string[]': {
      if (Array.isArray(value)) {
        return value.map((element: unknown) => String(element));
      }
      const str = String(value);
      return str.split(',').map((element: string) => element.trim());
    }

    case 'number[]': {
      let elements: string[];
      if (Array.isArray(value)) {
        elements = value.map((element: unknown) => String(element));
      } else {
        const str = String(value);
        elements = str.split(',').map((element: string) => element.trim());
      }
