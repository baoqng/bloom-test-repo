...
export function parseRetryAfter(header: unknown): number {
  ...
  // Check if it looks like an integer string (optional leading minus, digits only)
  if (/^-?\d+$/.test(trimmed)) {
    // Reject leading zeros for multi-digit numbers
    if (/^-?0\d+/.test(trimmed)) {
      throw new RangeError('header is not a valid non-negative integer or parseable date');
    }

    const value = parseInt(trimmed, 10);

    if (value < 0) {
      throw new RangeError('header integer value must be non-negative');
    }
